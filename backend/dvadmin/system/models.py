...
from dvadmin.plugins.models import CoreModel, table_prefix
...
class Users(AbstractUser, CoreModel):
    ...
    dept = models.ForeignKey(to='Dept', verbose_name='所属部门', on_delete=models.CASCADE, db_constraint=False, null=True, blank=True,help_text="关联部门")
...
class Role(CoreModel):
    ...
    dept = models.ManyToManyField(to='Dept', verbose_name='数据权限-关联部门', db_constraint=False,help_text="关联部门")
...
class Dept(CoreModel):
    ...
    status = models.IntegerField(choices=STATUS_CHOICES,default=1, verbose_name="部门状态", null=True, blank=True,help_text="部门状态")
...
class MenuPermission(CoreModel):
    menu = models.ForeignKey(to="Menu",on_delete=models.CASCADE,verbose_name="关联菜单",help_text='关联菜单')
    ...
    class Meta:
        db_table = table_prefix+"menu_permission"
        verbose_name = '菜单权限表'
        verbose_name_plural = verbose_name
...
class Dictionary(CoreModel):
    ...
    sort = models.IntegerField(default=1,verbose_name="显示排序",null=True, blank=True,help_text="显示排序")
    parent = models.ForeignKey(to="Dictionary",db_constraint=False,on_delete=models.PROTECT,blank=True, null=True,verbose_name="父级",help_text="父级")
